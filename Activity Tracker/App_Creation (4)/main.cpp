...
#define MBED_APP_IOTC_DEVICE_PATH                                              \
  "projects/" MBED_CONF_APP_GOOGLE_CLOUD_PROJECT_ID                            \
  "/locations/" MBED_CONF_APP_GOOGLE_CLOUD_REGION                              \
  "/registries/" MBED_CONF_APP_GOOGLE_CLOUD_REGISTRY                           \
  "/devices/" MBED_CONF_APP_GOOGLE_CLOUD_DEVICE_ID

#define MBED_APP_MQTT_TOPIC                                                    \
  "/devices/" MBED_CONF_APP_GOOGLE_CLOUD_DEVICE_ID                             \
  "/events/" MBED_CONF_APP_GOOGLE_CLOUD_MQTT_TOPIC

#define MBED_APP_MQTT_SUB_TOPIC                                                \
  "/devices/" MBED_CONF_APP_GOOGLE_CLOUD_DEVICE_ID "/commands/#"
...
static void on_message_sent(iotc_context_handle_t in_context_handle, void *data,
                            iotc_state_t state) {
  if (state == IOTC_STATE_OK) {
    printf("Message sent successfully\n");
  } else {
    printf("Failed to send message, error: %s", iotc_get_state_string(state));
  }
}
...
static void recordMovement() { 
  ...
  BSP_ACCELERO_AccGetXYZ(data.accel);

  movementReadings.push_back(data);
}

static void publishMovements(iotc_context_handle_t context_handle) {
  std::string messageData = "{ \"readings\": [";

  bool first = true;
  for (const auto &md : movementReadings) {
    if (first) {
      first = false;
    } else {
      messageData += ", ";
...
//Json data structure
    ...
    messageData += "\"z\": " + std::to_string(md.accel[2]) + " }, ";
    messageData += "\"gyro\": { ";
    messageData += "\"x\": " + std::to_string(md.gyro[0]) + ", ";
    messageData += "\"y\": " + std::to_string(md.gyro[1]) + ", ";
    messageData += "\"z\": " + std::to_string(md.gyro[2]) + " } }";
  }
  ...
  iotc_state_t state =
      iotc_publish(context_handle, MBED_APP_MQTT_TOPIC, messageData.c_str(),
...
void publish_function(iotc_context_handle_t context_handle,
  ...
  if (movementReadings.size() >= 5) {
    publishMovements(context_handle);
...
void on_connection_state_changed(iotc_context_handle_t in_context_handle,
  ...
  case IOTC_CONNECTION_STATE_OPENED:
    ...
    iotc_subscribe(in_context_handle, MBED_APP_MQTT_SUB_TOPIC,
                   IOTC_MQTT_QOS_AT_LEAST_ONCE, &on_message_received, NULL);
    ...
    /* Create a timed task to publish every 1 seconds. */
    delayed_publish_task = iotc_schedule_timed_task(
        in_context_handle, publish_function, 1, 15, /*user_data=*/NULL);
    break;
  ...
  case IOTC_CONNECTION_STATE_CLOSED: {
    ...
    if (state == IOTC_STATE_OK) {
      ...
      state = iotc_create_iotcore_jwt(
          MBED_CONF_APP_GOOGLE_CLOUD_DEVICE_ID, JWT_EXPIRATION_PERIOD_SEC,
...
int ntp_client(NetworkInterface *net) {
  NTPClient ntp(net);
  ntp.set_server("0.pool.ntp.org", 123);
...
int main(int argc, char *argv[]) {
  ...
  iotc_connect_private_key_data.crypto_key_union.key_pem.key =
      google_cloud::credentials::clientKey;
  ...
  size_t bytes_written = 0;
  state = iotc_create_iotcore_jwt(
      MBED_CONF_APP_GOOGLE_CLOUD_PROJECT_ID, JWT_EXPIRATION_PERIOD_SEC,
      &iotc_connect_private_key_data, jwt, IOTC_JWT_SIZE, &bytes_written);
  ...
  state = iotc_connect(iotc_context, /*username=*/NULL, /*password=*/jwt,
                       /*client_id=*/MBED_APP_IOTC_DEVICE_PATH,
                       MQTT_CONNECTION_TIMEOUT_SEC, MQTT_KEEPALIVE_TIMEOUT_SEC,
                       &on_connection_state_changed);
